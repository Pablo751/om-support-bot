...
class SupportBot:
    def __init__(self):
        ...
        self.system_instructions = texts.SYSTEM_INSTRUCTIONS.format(knowledge=self.knowledge_service.load_and_build_knowledge())

    async def process_query(self, message):
        self.message = message
        logger.info(f"Sending requesst to OpenAI query: {self.message.query}")
        query_response = self.openai.analyze_query(self.system_instructions, self.message.query)
        ...
        if query_type == "STORE_STATUS":
            if not client_name or not commerce_id:
                ...
                if store_status is None:
                    ...
                    act_response = texts.STORE_ACTIVE_MSG.format(commerce_id=commerce_id, client_name=client_name)
                else:
                    act_response = texts.STORE_INACTIVE_MSG.format(commerce_id=commerce_id, client_name=client_name)
