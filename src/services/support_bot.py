...
class SupportBot:
    ...
    async def act(self, query_response):
        ...
        company_name = query_response.get("company_name")
        commerce_id = query_response.get("commerce_id")

        if query_type == "STORE_STATUS":
            if not company_name or not commerce_id:
                act_response = texts.STORE_STATUS_MISSING_MSG
            else:
                store_status = self.mongo_service.check_store_status(company_name, commerce_id)
                if store_status is None:
                    act_response = texts.STORE_NOT_FOUND_MSG
                elif store_status:
                    act_response = texts.STORE_ACTIVE_MSG.format(commerce_id=commerce_id, company_name=company_name)
                else:
                    act_response = texts.STORE_INACTIVE_MSG.format(commerce_id=commerce_id, company_name=company_name)
